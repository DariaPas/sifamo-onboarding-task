...
import com.sifamo.order.client.customer.api.DefaultApi;
...
public class CustomerServiceClientAdapter implements CustomerValidationPort {
    ...
    private final DefaultApi customerApi;

    public CustomerServiceClientAdapter() {
        ApiClient apiClient = new ApiClient();
        apiClient.setBasePath("http://localhost:8081");
        this.customerApi = new DefaultApi(apiClient);
    }

    @Override
    public boolean customerExists(UUID customerId) {
        try {
            customerApi.customersIdGet(customerId);
            return true;
    ...
    public boolean shippingAddressExists(UUID customerId, UUID shippingAddressId) {
        try {
            customerApi.customersIdShippingAddressesShippingAddressIdGet(customerId, shippingAddressId);
            return true;
        } catch (HttpClientErrorException.NotFound exception) {
            return false;
        }
